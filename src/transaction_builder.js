...
TransactionBuilder.prototype.buildHashForSignature =function (vin, keyPair, redeemScript, hashType, witnessValue, witnessScript){
  // TODO: remove keyPair.network matching in 4.0.0
  if (keyPair.network && keyPair.network !== this.network) throw new TypeError('Inconsistent network')
  if (!this.inputs[vin]) throw new Error('No input at index: ' + vin)
  hashType = hashType || Transaction.SIGHASH_ALL
...
TransactionBuilder.prototype.fillSignature = function(vin, keyPair, signature, hashType){
  var input = this.inputs[vin]
  var kpPubKey = keyPair.publicKey || keyPair.getPublicKeyBuffer()
  hashType = hashType || Transaction.SIGHASH_ALL
  var signed = input.pubKeys.some(function (pubKey, i) {
    ...
    if (Buffer.isBuffer(signature)) signature = ECSignature.fromRSBuffer(signature)
    console.log(keyPair.verify(signatureHash,signature))

    input.signatures[i] = signature.toScriptSignature(hashType)
    return true
  })
  if (!signed) throw new Error('cannot sign for this input')
}
